...
import random
class ifs_function():
    def __init__(self, prev_x, prev_y, save_root,fractal_name,fractal_weight_count, mask_mode='random'):
        # previous (x, y)
    ...
    def draw_point(self,image_x,image_y,pad_x,pad_y,set_color,count, threshold = 0.4):
    ...
    def draw_patch(self,image_x,image_y,pad_x,pad_y,set_color,count, threshold = 0.1, imgsave=True):
        self.__rescale(image_x,image_y,pad_x,pad_y)
        ...
        for i in range(len(self.xs)):
            ...
            elif self.mask_mode == 'fill':
                mask_pattern = '111111111'
            else:
                raise ValueError(f'{self.mask_mode} is not defined!')
        ...
        white_pixels_rate = np.sum(img_data > 1) / img_data.size
        if white_pixels_rate > threshold:
            result = True
        ...
        if result:
            ...
            if imgsave:
                ...
                trans_image.close()
                image.close()
                print(os.path.join(self.save_root, self.fractal_name, self.fractal_name + "_" + self.fractal_weight_count + "_count_" + str(count) + "_flip" + str(trans_type) + ".png"))
            return image
        else:
            image.close()
            return False
